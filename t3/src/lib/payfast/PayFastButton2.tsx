export function PayFastButton2(props: {
  paymentUuid: string;
  ...
  return (
    <form action={options.action_url} method="POST">
      ...
      <input
        type="hidden"
        name="notify_url"
        value={window.location.origin + "/api/payment/notify2"}
      />
      ...
      >
        Pay & Publish {props.sandbox && "sandbox"}
      </button>
    </form>
  );
}
