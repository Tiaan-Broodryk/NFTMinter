...
export function PayFastButton(props: PayFastButtonProps) {
  return (
    <NoSSR>
...
export function PayFastButtonInside(props: PayFastButtonProps) {
  ...
  const myData: RouterInputs["lib"]["payfast"]["generate_signature"] = {
    merchant_id: payfast_info.data?.merchant_id ?? "",
    return_url: window.location.origin,
    cancel_url: window.location.origin,
    email_address: props.paymentEmail,
    name_first: props.paymentFirstName,
    name_last: props.paymentLastName,
    item_name: props.item_name,

    m_payment_id: props.m_payment_id,
  ...
  if (props.subscription_type === "subscription") {
    ...
    if (props.recurring_amount)
      myData.recurring_amount = props.recurring_amount.toFixed(2);
  ...
  const signature = api.lib.payfast.generate_signature.useQuery(myData, {
  ...
  return (
    <form action={payfast_info.data.action_url} method="POST">
