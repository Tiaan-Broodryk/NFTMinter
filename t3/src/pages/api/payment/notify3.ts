...
const handler = async (
  ...
  if (req.body.payment_status === "COMPLETE") {
    ...
    const update = req.body.custom_str4
      .split(",")
      .map(
        async (i) =>
          await dbc.client.query(
            /* surrealql */ `UPDATE vehicle:${i.split(":")[1]!} SET published = true,publish_date = d"${today.toISOString()}", exp_date = d"${exp_date.toISOString()}",status = "active" ,Ad_paid_date = d"${today.toISOString()}",can_expire = true;`,
          ),
      );
  }
